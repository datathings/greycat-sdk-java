...
JNIEXPORT jstring JNICALL Java_io_greycat_impl_DateImpl_nToString(JNIEnv *env, jclass class, jlong value) {
    ...
    if (rest == 0) {
        ...
        char buffer[len + 1];
        snprintf(buffer, len + 1, GDATE_ISO_FMT_Z, (uint32_t) epoch_tm.tm_year + 1900, epoch_tm.tm_mon, epoch_tm.tm_mday, epoch_tm.tm_hour, epoch_tm.tm_min,
                 epoch_tm.tm_sec);
        buffer[len] = '\0';
        return (*env)->NewStringUTF(env, buffer);
    } else {
        uint32_t len = (uint32_t) snprintf(NULL, 0, GDATE_ISO_FMT_ZR, epoch_tm.tm_year + 1900, epoch_tm.tm_mon, epoch_tm.tm_mday, epoch_tm.tm_hour,
                                           epoch_tm.tm_min, epoch_tm.tm_sec, rest);
        char buffer[len + 1];
        snprintf(buffer, len + 1, GDATE_ISO_FMT_ZR, (uint32_t) epoch_tm.tm_year + 1900, epoch_tm.tm_mon, epoch_tm.tm_mday, epoch_tm.tm_hour, epoch_tm.tm_min,
